<?php
  ...
  $queryOradoresSelecionados = "SELECT utilizador_id FROM conferencia_has_orador WHERE conferencia_id = ?";
  $stmt = mysqli_prepare($connectionDB, $queryOradoresSelecionados);
  mysqli_stmt_bind_param($stmt, "i", $conferenciaId);
  mysqli_stmt_execute($stmt);
  $resultOradoresSelecionados = mysqli_stmt_get_result($stmt);
  $oradoresSelecionados = array_column(mysqli_fetch_all($resultOradoresSelecionados, MYSQLI_ASSOC), 'utilizador_id');
  mysqli_stmt_close($stmt);
        ...
        <form method="POST" action="">
            ...
            <div class="form-group">
                ...
                <?php foreach ($oradores as $orador): ?>
                    <label>
                        <input type="checkbox" name="oradores[]" value="<?php echo $orador['id']; ?>"
                            <?php echo in_array($orador['id'], $oradoresSelecionados) ? 'checked' : ''; ?>>
                        <?php echo htmlspecialchars($orador['nome']); ?>
                    </label><br>
                <?php endforeach; ?>
            </div>
            <div class="form-group">
                ...
                <?php foreach ($participantes as $participante): ?>
                    <label>
                        <input type="checkbox" name="participantes[]" value="<?php echo $participante['id']; ?>"
                            <?php echo in_array($participante['id'], $participantesSelecionados) ? 'checked' : ''; ?>>
